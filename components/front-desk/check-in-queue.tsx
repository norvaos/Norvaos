...
function getIdStatus(
  ...
  if (checkInStatus === 'identity_verified' || checkInStatus === 'completed') {
    return { label: 'Verified', color: 'bg-emerald-950/30 text-emerald-400 border-emerald-500/20' }
  }

  if (idScanUploadedAt) {
    return { label: 'Uploaded', color: 'bg-blue-950/30 text-blue-400 border-blue-500/20' }
  }

  return { label: 'Pending', color: 'bg-amber-950/30 text-amber-400 border-amber-500/20' }
}
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {ci.contact_id && onSelectContact ? (
                          <button
                            type="button"
                            className="text-sm font-medium text-slate-900 hover:text-blue-400 transition-colors truncate block"
                    ...
                    {isActionable && (
                      <div className="flex items-center gap-1.5">
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-7 gap-1 text-xs text-slate-600 hover:text-blue-400 hover:bg-blue-950/30"
                        ...
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-7 gap-1 text-xs text-slate-600 hover:text-emerald-400 hover:bg-emerald-950/30"
