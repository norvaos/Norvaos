...
const KYC_CONFIG: Record<KycStatus, { label: string; colour: string; icon: typeof ShieldCheck }> = {
  verified: { label: 'Verified', colour: 'border-emerald-500/30 bg-emerald-950/30 text-emerald-400', icon: ShieldCheck },
  pending: { label: 'Pending', colour: 'border-amber-500/30 bg-amber-950/30 text-amber-400', icon: Shield },
...
const CONFLICT_CONFIG: Record<ConflictPulse, { label: string; colour: string; icon: typeof ShieldCheck }> = {
  passed: { label: 'Passed', colour: 'border-emerald-500/30 bg-emerald-950/30 text-emerald-400', icon: ShieldCheck },
  flagged: { label: 'Flagged', colour: 'border-red-500/30 bg-red-950/30 text-red-400', icon: ShieldAlert },
  not_started: { label: 'Not Started', colour: 'border-slate-200 bg-slate-50 text-slate-500', icon: Shield },
}

const RETAINER_CONFIG: Record<RetainerPulse, { label: string; colour: string; icon: typeof FileSignature }> = {
  hash_verified: { label: 'Hash-Verified', colour: 'border-emerald-500/30 bg-emerald-950/30 text-emerald-400', icon: FileSignature },
  signed: { label: 'Signed', colour: 'border-blue-500/30 bg-blue-950/30 text-blue-400', icon: FileSignature },
  unsigned: { label: 'Unsigned', colour: 'border-amber-500/30 bg-amber-950/30 text-amber-400', icon: FileSignature },
...
const AML_CONFIG: Record<AmlPulse, { label: string; colour: string; icon: typeof Fingerprint }> = {
  match: { label: 'Match', colour: 'border-emerald-500/30 bg-emerald-950/30 text-emerald-400', icon: Fingerprint },
  mismatch: { label: 'Mismatch', colour: 'border-red-500/30 bg-red-950/30 text-red-400 animate-pulse', icon: Fingerprint },
...
function scoreBorderColour(score: number): string {
  ...
  if (score >= 75) return 'border-blue-500/30'
