 ...
 * - Pinned, Published, and Draft sections
 * - Search with debounce
 * - New Playbook button → /wiki/playbooks/new
 */

import { useState, useEffect, useCallback } from 'react'
import { useRouter } from 'next/navigation'
import { useTenant } from '@/lib/hooks/use-tenant'
import { useWikiPlaybooks, useWikiCategories } from '@/lib/queries/wiki'
import { cn } from '@/lib/utils'
import { BookOpen, Search, Star, Plus, ArrowLeft, Tag } from 'lucide-react'
...
export default function PlaybooksPage() {
  ...
  const { data: playbooks = [], isLoading } = useWikiPlaybooks({
    tenantId,
  ...
  const pinned = playbooks.filter(p => p.is_pinned)
  const published = playbooks.filter(p => p.status === 'published' && !p.is_pinned)
  const drafts = playbooks.filter(p => p.status === 'draft')
          ...
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-muted-foreground mt-1 ml-9">
                Your firm&apos;s operational playbooks and standard operating procedures.
              </p>
            </div>
            <button
            ...
            <>
              ...
              {playbooks.length === 0 && (
                <div className="rounded-xl border-2 border-dashed border-border p-12 text-center">
                  ...
                  <h3 className="text-lg font-semibold text-foreground/80 mb-1">No Playbooks Found</h3>
                  <p className="text-sm text-muted-foreground mb-4">
                    {search.trim()
                      ? `No playbooks match "${search}". Try a different search term.`
