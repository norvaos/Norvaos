...
interface RouteParams {
  params: Promise<{ matterTypeId: string }>
}

/**
 * GET /api/ircc/matter-types/[matterTypeId]/forms
 *
 * Get all IRCC forms assigned to a matter type, ordered by sort_order.
 * Includes full form details.
 */
export async function GET(_request: Request, { params }: RouteParams) {
  try {
    const auth = await authenticateRequest()
    requirePermission(auth, 'form_packs', 'view')
    const { matterTypeId } = await params
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const supabase: any = createAdminClient()

    // Verify the matter type belongs to this tenant
    ...
    if (!matterType) {
      return NextResponse.json({ error: 'Matter type not found' }, { status: 404 })
    }
    ...
    console.error('[ircc-matter-types/forms] GET Error:', error)
...
/**
 * POST /api/ircc/matter-types/[matterTypeId]/forms
 *
 * Add an IRCC form to a matter type.
 *
...
export async function POST(request: Request, { params }: RouteParams) {
  try {
    ...
    // Verify matter type belongs to tenant
    const { data: matterType } = await supabase
      .from('matter_types')
      .select('id')
      .eq('id', matterTypeId)
    ...
    // Insert stream-form assignment with matter_type_id
    ...
    if (error) {
      ...
      if ((error as any).code === '23505') {
        return NextResponse.json(
          { error: 'This form is already assigned to this matter type' },
          { status: 409 },
        )
      }
      return NextResponse.json({ error: (error as Error).message }, { status: 500 })
    ...
    console.error('[ircc-matter-types/forms] POST Error:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Internal server error' },
      { status: 500 },
    )
...
/**
 * DELETE /api/ircc/matter-types/[matterTypeId]/forms
 *
 * Remove an IRCC form from a matter type.
 *
...
export async function DELETE(request: Request, { params }: RouteParams) {
  try {
    ...
    console.error('[ircc-matter-types/forms] DELETE Error:', error)
    return NextResponse.json(
      { error: error instanceof Error ? error.message : 'Internal server error' },
      { status: 500 },
    )
