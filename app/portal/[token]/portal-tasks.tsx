...
export function PortalTasks({ token, primaryColor, language = 'en' }: PortalTasksProps) {
  ...
  if (tasks.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-12 text-center">
        <div className="mx-auto h-12 w-12 rounded-2xl bg-gradient-to-br from-slate-50 to-white flex items-center justify-center mb-3 shadow-sm" style={{ backgroundColor: `${primaryColor || '#2563eb'}08` }}>
          <svg className="h-5 w-5" style={{ color: primaryColor || '#2563eb' }} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
            <path d="M9 11l3 3L22 4" />
            <path d="M21 12v7a2 2 0 01-2 2H5a2 2 0 01-2-2V5a2 2 0 012-2h11" />
          </svg>
        </div>
        <p className="text-sm font-medium text-slate-500">{tr.no_tasks ?? 'No tasks assigned at this time.'}</p>
      </div>
    )
  }
                      ...
                      {groupKey === 'todo' && (
                        <div className="flex items-center gap-1.5">
                          <button
                            ...
                            className="rounded-xl border border-slate-200 bg-white px-3 py-1.5 text-xs font-semibold text-slate-700 transition-all hover:bg-slate-50 hover:shadow-sm disabled:opacity-50"
                          >
                            {updatingId === task.id ? (
                          ...
                          <button
                            ...
                            disabled={updatingId === task.id}
                            className="rounded-xl px-3 py-1.5 text-xs font-bold text-white transition-all disabled:opacity-40 shadow-sm hover:brightness-110 active:scale-[0.97]"
