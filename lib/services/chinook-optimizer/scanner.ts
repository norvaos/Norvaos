/**
 * Regulator-Mirror 3.0 Pre-Submission IRCC AI-Readability Scanner
 *
 * Analyses document text against the IRCC keyword taxonomy,
 * checks structural indicators, and produces a readability score
 * with actionable recommendations to maximise IRCC throughput.
 */
...
export interface IRCCScanResult {
...
const CASE_TYPE_CATEGORY_MAP: Record<string, IRCCCategory[]> = {
 ...
 * are returned.
 */
export function getRelevantCategories(caseType?: string): IRCCCategory[] {
  if (!caseType) {
    return Object.keys(IRCC_KEYWORDS) as IRCCCategory[];
  }
...
function analyseStructure(text: string): StructureIssue[] {
  ...
  if (!hasTocIndicators) {
    issues.push({
      ...
      severity: 'critical',
      description:
        "No table of contents or bookmark indicators detected. IRCC's automated system uses bookmarks to navigate sections.",
  ...
  if (!hasCoverLetterFirst) {
    issues.push({
      ...
      severity: 'critical',
      description:
        "The opening section does not appear to be a cover letter. IRCC's automated system expects the cover letter as the first document.",
  ...
  if (!hasAnnexReferences) {
    issues.push({
      ...
      severity: 'info',
      description:
        "No explicit annex or exhibit references found. Cross-references help IRCC's automated system map evidence to arguments.",
...
/**
 * Scans document text for IRCC's automated system AI-readability and returns a
 ...
 */
export function scanDocument(text: string, caseType?: string): IRCCScanResult {
  ...
  for (const cat of categories) {
    const keywords = IRCC_KEYWORDS[cat];
  ...
  for (const zoneName of zoneNames) {
    ...
    for (const cat of categories) {
      for (const keyword of IRCC_KEYWORDS[cat]) {
  ...
  if (coverageRatio < 0.2) {
    ...
    recommendations.push({
      ...
      action:
        "Add missing IRCC-standard terms to strengthen IRCC's automated system readability. Focus on the categories most relevant to this application type.",
    });
  }

  // Missing category recommendations
  for (const cat of categories) {
    const catKeywords = IRCC_KEYWORDS[cat];
  ...
  if (zoneAnalysis.hits.opening_paragraph === 0) {
    recommendations.push({
      ...
      action:
        "The first 200 words carry high weight in IRCC's automated system. State the application type, legal basis, and key facts upfront.",
    });
  }
