...
export async function convertLeadToMatter(
  ...
  const idempotentResult = await executeIdempotent<{ matterId: string }>(supabase, {
    ...
    handler: async () => {
      ...
      // 5n + 5o: Fire-and-forget — document request email + OneDrive sync
      // These are non-blocking so the conversion response returns immediately.
      const backgroundTasks = async () => {
        // Send document request if doc slots were generated by kit
        try {
        ...
        // OneDrive folder structure + document migration
        try {
      ...
      // Don't await — let email + OneDrive run after response is sent
      backgroundTasks().catch((err) =>
        console.error('[conversion] Background tasks error:', err)
      )
