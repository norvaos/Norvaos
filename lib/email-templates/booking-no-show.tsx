...
interface BookingNoShowEmailProps {
  firmName: string
...
export function BookingNoShowEmail({
  ...
  lawyerName, appointmentDate, appointmentTime,
}: BookingNoShowEmailProps) {
  const greeting = clientFirstName ? `Dear ${clientFirstName}` : 'Dear Client'

  return (
    <Html>
      <Head />
      <Preview>Missed appointment on {appointmentDate}</Preview>
        ...
        <Container style={containerStyle}>
          ...
          <Section style={contentStyle}>
            ...
            <Text style={paragraphStyle}>
              We noticed you were unable to attend your scheduled appointment. Here were the details:
            </Text>
            ...
            <Text style={paragraphStyle}>
              We understand that things come up. If you would like to reschedule, please don&apos;t hesitate to contact us.
            </Text>
...
export async function renderBookingNoShowEmail(props: BookingNoShowEmailProps): Promise<{
  html: string; text: string; subject: string
}> {
  const html = await render(<BookingNoShowEmail {...props} />)
  const text = [
  ...
  return { html, text, subject: `Missed Appointment: ${props.appointmentDate}` }
}
